        ...
        <div class="content-wrapper">
            ...
            <div class="report-content">
                <div class="content-group">
                    <h2 class="content-title">STAFF ON SITE</h2>
                    @php
                        $staff = $report->getStuffData();
                    @endphp
                    @if (!empty($staff))
                            ...
                            <tbody>
                                @foreach ($staff as $staffUser)
                                    <tr>
                                        ...
                                        <td>{{ $staffUser->user->first_name }} {{ $staffUser->user->last_name }}</td>
                                        <td>{{ $staffUser->user->company }}</td>
                                        <td>{{ \Carbon\Carbon::parse($staffUser->entry_time)->format('h:i A') }}</td>
                                        <td>{{ \Carbon\Carbon::parse($staffUser->exit_time)->format('h:i A') }}</td>
                                    </tr>
                                @endforeach
                            </tbody>
                        </table>
                ...
                <div class="content-group">
                    <h2 class="content-title">EQUIPMENT ON SITE</h2>
                    <p class="equipments">
                        @foreach ($report->getSiteDiaries() as $siteDiary)
                            @foreach ($siteDiary->equipments as $equipment)
                                {{ $equipment->equipment }},
                            @endforeach
                        @endforeach
                    </p>
                            ...
                            <tr>
                                @foreach ($report->getSiteDiaries() as $siteDiary)
                                    @foreach ($siteDiary->images as $image)
                                        <td class="col-3">
                                            <img src="data:image/jpeg;base64,{{ base64_encode(Storage::get($image->url)) }}"
                                                alt="Logo">
                                        </td>
                                    @endforeach
                                @endforeach
